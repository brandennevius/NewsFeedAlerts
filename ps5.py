...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        ...
        no_punctuation_text = ''.join(char if char not in string.punctuation else ' ' 
                for char in lower_text)

        clean_text = ' '.join(no_punctuation_text.split()) + ' '

        no_punctuation_phrase = ''.join(char if char not in string.punctuation else ' '
                for char in lower_phrase)

        cleaned_phrase = ' '.join(no_punctuation_phrase.split()) + ' '
        
        # Search cleaned text for phrase
        if cleaned_phrase not in clean_text:
            return False
        else:
            return True
    ...
    for i in range(len(lines)):
        ...
        if trig[1] == 'TITLE':
            ...
            for x in range(1, len(trig)):
                triggers_list.append(trigger_dictionary[trig[x]])
